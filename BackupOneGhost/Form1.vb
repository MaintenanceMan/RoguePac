Public Class Form1
    ...
    Dim gGhostX As Integer
    Dim gGhostY As Integer
    Dim gGhostHP As Integer
    ...
    Private Sub InitGhost()
        ...
        gGhostX = nX
        gGhostY = nY
        gArrMaze(gGhostY, gGhostX) = gGHOST
        gGhostHP = gSTART_HIT_POINTS
    End Sub
    ...
    Private Sub EraseGhost()
        gArrMaze(gGhostY, gGhostX) = gEMPTY
    End Sub

    Private Function CanMove(ByVal nX, ByVal nY) As Boolean
        Dim bCanMove As Boolean
        bCanMove = False
        If (gArrMaze(nY, nX) = gEMPTY) Then
            bCanMove = True
        End If
        Return bCanMove
    End Function
    ...
    Private Sub Form1_Paint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles MyBase.Paint
        ...
        For nRow = 0 To gMAX_ROWS - 1
            ...
            strRow = ""
            For nCol = 0 To gMAX_COLS - 1
                strRow += gArrMaze(nRow, nCol)
            Next
            e.Graphics.DrawString(strRow, fontObj, Brushes.White, 0, nRow * 16)
        Next
        ' Show stats
        Dim strStats As String
        strStats = "HP: " + CStr(gPlayerHP)
    ...
    Private Sub MoveGhost(ByVal nX As Integer, ByVal nY As Integer)
        EraseGhost()
        gGhostX = nX
        gGhostY = nY
        gArrMaze(gGhostY, gGhostX) = gGHOST
    ...
    Private Sub ProcessGhost()
        Dim nNewX As Integer
        Dim nNewY As Integer
        nNewX = gGhostX
        nNewY = gGhostY
        ' Determine optimal move toward player
        If (gPlayerX > gGhostX) Then
            nNewX = nNewX + 1
        ElseIf (gPlayerX < gGhostX) Then
            nNewX = nNewX - 1
        End If
        If (gPlayerY > gGhostY) Then
            nNewY = nNewY + 1
        ElseIf (gPlayerY < gGhostY) Then
            nNewY = nNewY - 1
        End If
        If CanMove(nNewX, nNewY) Then
            MoveGhost(nNewX, nNewY)
    ...
    Private Sub Regenerate()
        ...
        If (gPlayerHP > gMAX_HIT_POINTS) Then
            gPlayerHP = gMAX_HIT_POINTS
        End If
    End Sub
    ...
    Private Sub ProcessPlayer(ByVal nX As Integer, ByVal nY As Integer)
        ...
        ElseIf IsGhost(nX, nY) Then
            gGhostHP = gGhostHP - gPLAYER_DAMAGE
